...
/**
 * Pages this build produces, mirroring the rollup inputs in vite.config.ts.
...
export function precachePages(): string[] {
  return PAGES;
}
...
/**
 * Same-origin paths the worker must never serve from its own cache. The vote
 * and request endpoints are rate limited per request and must stay live; the
 * gated control app and admin dashboard live in the separate openmouse repo
 * and are never emitted here, but the patterns are kept defensively in case
 * this site is ever proxied behind the same edge as that app.
 */
export const BYPASS = [
  /^\/api\//,
  /^\/contributors\.json$/,
...
const CACHE = "openmouse-landing-${version}";
...
/**
 * Opaque font responses carry no readable headers, so the directive check
 * passes them through and they stay cacheable despite reporting ok === false.
 */
  ...
  return {
    ...
    generateBundle: {
      ...
      handler(_options, bundle) {
        ...
        for (const file of precachePages()) {
